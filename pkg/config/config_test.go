...
func TestNoRedis(t *testing.T) {
	_, err := validate("/s.rump", "/t.rump", false, false, 0)
	if err == nil {
...
func TestNoFrom(t *testing.T) {
	_, err := validate("", "redis://t", false, false, 0)
	if err == nil {
...
func TestNoTo(t *testing.T) {
	_, err := validate("redis://s", "", false, false, 0)
	if err == nil {
...
func TestFromRedisToRedis(t *testing.T) {
	cfg, err := validate("redis://s", "redis://t", false, false, 0)
	if err != nil {
...
func TestFromRedisToFile(t *testing.T) {
	cfg, err := validate("redis://s", "/t.rump", false, false, 0)
	if err != nil {
...
func TestFromFileToRedis(t *testing.T) {
	cfg, err := validate("/s.rump", "redis://t", false, false, 0)
	if err != nil {
...
func TestFromRedisToRedisWithAuth(t *testing.T) {
	cfg, err := validate("redis://:a@s", "redis://:b@t", false, false, 0)
	if err != nil {
		t.Error("from redis to redis should work")
	}
